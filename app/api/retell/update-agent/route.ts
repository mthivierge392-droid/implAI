...
export async function PATCH(request: NextRequest) {
  ...
  try {
    ...
    const { agent_id, voice_id, agent_name } = updateAgentSchema.parse(body);
    console.log('Parsed agent_id:', agent_id, 'voice_id:', voice_id, 'agent_name:', agent_name);
    ...
    if (!response.ok) {
      ...
      if (response.status === 404) {
        ...
        await serviceSupabase.from('agents').update({ voice: voice_id }).eq('id', agent.id);
    ...
    // Update voice in database only if voice_id was provided
    if (voice_id) {
